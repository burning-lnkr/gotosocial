...
// DomainSilencesPOSTHandler swagger:operation POST /api/v1/admin/domain_silences domainSilenceCreate
//
// Create one or more domain silences, from a string or a file.
//
// You have two options when using this endpoint: either you can set `import` to `true` and
// upload a file containing multiple domain silences, JSON-formatted, or you can leave import as
// `false`, and just add one domain silence.
//
...
func (m *Module) DomainSilencesPOSTHandler(c *gin.Context) {
	m.createDomainPermissions(c,
		gtsmodel.DomainPermissionSilence,
